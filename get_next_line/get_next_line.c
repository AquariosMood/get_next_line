...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0
		|| initialize_buffer(fd, &buffer, &temp) != 0)
	{
		free(temp);
		return (NULL);
	}
	result = read_and_update_buffer(fd, &buffer, temp);
	if (result == -1)
	{
		free(buffer);
		buffer = NULL;
	...
	if (result == 0)
		return (handle_buffer_result(&buffer, temp));
	return (process_line(&buffer, find_newline(buffer), temp));
}
